...
/**
 * @struct imgfs_header
 * @brief Defines the header for the image file system containing global settings and metadata.
 *
 * This structure encapsulates the core settings of the image file system such as the name,
 * version, and file count limitations, as well as dimensions for resized images.
 *
 * @param name        The name of the database, with space for a null terminator.
 * @param version     The version of the database useful for managing updates and compatibility.
 * @param nb_files    The current number of images in the file system.
 * @param max_files   The maximum number of images that the system can contain.
 * @param resized_res Array storing the resolutions of the "thumbnail" and "small" images.
 * @param unused_32   Reserved 32-bit integer for future use or alignment purposes.
 * @param unused_64   Reserved 64-bit integer for future use or alignment purposes.
 */
struct imgfs_header {
...
/**
 * @struct img_metadata
 * @brief Holds metadata for a single image within the image file system.
 *
 * This structure includes identification, integrity verification via SHA, original resolution,
 * and file management details for various image resolutions.
 *
 * @param img_id    Unique identifier for the image, null-terminated.
 * @param SHA       SHA256 hash for the image, used for integrity checks and deduplication.
 * @param orig_res  The resolution of the original image.
 * @param size      Sizes of the image files for each resolution.
 * @param offset    The positions in the "image database" file of images at the various possible resolutions.
 * @param is_valid  Flag to indicate if the image is in use (NON_EMPTY) or not (EMPTY).
 * @param unused_16 Reserved 16-bit integer for future use or alignment purposes.
 */
struct img_metadata {
...
/**
 * @struct imgfs_file
 * @brief Represents the image file system, encapsulating the file handle, header, and metadata.
 *
 * This structure is the top-level representation of an open image file system, linking together
 * the system's settings, the stored images' metadata, and the file handle used for operations.
 *
 * @param file     File pointer to the open file system file used for read/write operations.
 * @param header   The general information ("header") of the image database.
 * @param metadata Pointer to dynamically allocated array of the "metadata" of the images in the database.
 */
struct imgfs_file {
    FILE* file ;
